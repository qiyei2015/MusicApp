package com.qiyei.network.server.retrofit;
...
import com.qiyei.network.server.okhttp.ProgressInterceptor;
...
public class RetrofitEngine implements IHttpEngine {
    ...
    public <T, R> void enqueueGetCall(final HttpTask<T> task, final IHttpCallback<R> callback) {
        ...
        Object params = HttpUtils.gsonToGetParams(task.getRequest());

        //构造Call
        Call call = buildCall(task,params);
        if (call == null){
            callback.onFailure(new Exception("build call fail"));
            return ;
        }
        //设置task到okHttp拦截器中
        // TODO: 2020/4/18
...
//                Log.i(HttpManager.TAG, "callFactoryField is null");
//                return;
//            }
...
//        }
    ...
    private <T> Call buildCall(HttpTask<T> task,Object params){
        ...
        if (TextUtils.isEmpty(methodName)){
            Log.i(HttpManager.TAG,"cannot find method in " + task.getRequest().getApiClazz());
            return null;
        }
...
//                params = new Object[0];
            ...
            Log.i(HttpManager.TAG,"params:" + params);
    ...
    private String getMethodName(HttpTask task){
        ...
        for (Method method : task.getRequest().getApiClazz().getDeclaredMethods()){
            ...
            if (value != null && task.getRequest().getPathUrl().equals(value)){
                ...
                Log.v(HttpManager.TAG,"method :" + methodName);
            }
        }
    ...
    private Class<?> getParamsClazz(Class<?> targetClazz,Class<?> paramsClazz,String name){
        ...
        for (Method method : targetClazz.getDeclaredMethods()){
            ...
            if (name.equals(method.getName())){
                Log.v(HttpManager.TAG,"paramsClazz :" + paramsClazz.getName());
                //目前只考虑只有一个参数的
...
//                Log.i(HttpManager.TAG,"interceptorList is null or size is 0");
//                return;
//            }
//            //找到Interceptor
//            MyInterceptor interceptor = (MyInterceptor) interceptorList.get(0);
//            if (interceptor == null){
//                Log.i(HttpManager.TAG,"MyInterceptor is null ");
//                return;
//            }
//            interceptor.setProgressResponseBody(new ProgressResponseBody(callback));
    ...
    private void test(Class<?> clazz,String methodName,String annotationValue,Class<?>... parameterTypes){
        ...
        try {
            ...
            for (Field field : handler.getClass().getDeclaredFields()){
                Log.i(HttpManager.TAG,"field:" + field.getName() + "  type:" + field.getType().getName());
            }
            ...
            if (type.isArray()){
                Log.i(HttpManager.TAG,"length:" + Array.getLength(objects));
                ...
                Log.i(HttpManager.TAG,"annotationValue:" + annotationValue);
